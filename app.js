...
function renderPitches(){
  ...
  grid.innerHTML = list.map((p)=>{
    ...
    const canDelete = p.mine || (profile && profile.is_admin);
    const deleteBtn = canDelete ? `<button class="vote-btn" data-delpitch="${p.id}" title="Delete this pitch" style="color:var(--danger);border-color:rgba(248,113,113,.3);">🗑</button>` : '';
    return `
      <article class="card">
    ...
    return `
      <div class="squad-card">
        ...
        <div class="squad-main" style="width:100%;">
          ...
          ${requestsPanel}
          ${canDelete ? `<div style="margin-top:4px;">${deleteBtn}</div>` : ''}
        </div>
