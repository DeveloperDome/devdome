...
async function loadPitches(){
  ...
  pitches = pitchRows.map(p=>{
    ...
    // Use String() on both sides: Supabase returns bigint pitch IDs as numbers
    // for the pitches table but as strings for votes.pitch_id (or vice-versa
    // depending on JS client version). String coercion makes === always work.
    const pid = String(p.id);
    const votesForPitch = (voteRows||[]).filter(v=>String(v.pitch_id)===pid);
    return {
      id: p.id,
      title: p.title,
      desc: p.description,
      ...
      // author_id and profile.id are both UUIDs (strings) — compare with ===
      // but guard against null so mine is never accidentally true when logged out.
      mine: !!(myId && myId === p.author_id),
