...
/* ─── CUSTOM PRIVATE VISITOR COUNTER ──────────────────────────── */
async function trackAndDisplayViews() {
  try {
    // 1. Log this new visit to your database using your 'sb' client
    await sb.from('site_analytics').insert([{}]);

    // 2. Fetch the total number of rows (total views)
    const { count, error } = await sb
      .from('site_analytics')
      .select('*', { count: 'exact', head: true });

    if (!error && count) {
      console.log(`Total website visits: ${count}`);
    }
  } catch (err) {
...
// Run the counter automatically when the page loads
window.addEventListener('DOMContentLoaded', trackAndDisplayViews);
