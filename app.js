...
const firstLoadPromise = new Promise(res => { resolveFirstLoad = res; });
...
sb.auth.onAuthStateChange((event, newSession)=>{
  ...
  refreshing = refreshing
    ...
    .then(setupRealtime)
    .catch(err => { console.error('[DevDome] refresh failed:', err); })
    .finally(() => {
      console.log('[DevDome] refresh chain settled, firstLoadDone was:', firstLoadDone);
      if(!firstLoadDone){ firstLoadDone = true; resolveFirstLoad(); }
    });
});
console.log('[DevDome] onAuthStateChange listener registered');

// Safety net: if Supabase never calls back at all (bad URL/key, network
// block, CORS, etc.) don't leave the user staring at a spinner forever.
const bootTimeout = new Promise(res => setTimeout(res, 8000));

async function boot(){
  console.log('[DevDome] boot() called, waiting on race...');
  await Promise.race([firstLoadPromise, bootTimeout]);
  console.log('[DevDome] race resolved, revealing app');
  booted = true;
  document.getElementById('appLoading').hidden = true;
  document.getElementById('appRoot').hidden = false;
  if(!firstLoadDone){
    toast('Could not connect to the database. Check your Supabase URL/key and that the project is active.','warn');
  }
}

console.log('[DevDome] calling boot()');
boot();